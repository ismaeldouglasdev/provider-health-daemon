...
PROBER_INTERVAL_SECONDS = 90       # how often to probe routers for health (was 30s — cycle ≈ probe(≤60s)+30s left the /v1/models of 9router hammered on a 2-core host; 90s keeps the adapter cycle ≈ 2min, plenty for health)
...
PROBE_MAX_WORKERS = 3              # thread pool size for parallel probes (was 5 — 2 routers only; fewer concurrent /v1/models hits on slow host)
